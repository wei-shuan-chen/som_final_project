    ...
    for (int y = 0; y < infdata.resolution[2]; y++)
    {
        for (int x = 0; x < infdata.resolution[1]; x++)
        {
            for (int z = 0; z < infdata.resolution[0]; z++)
            {
                ...
                {
                    voxelModel.innerVoxel.push_back(USVoxData_t{{x, y, z}, {1.0, 0.2, 0.7}, {}});
...
bool RAWmodel_cls::choice_somvoxel(glm::mat3x3 m_inverse, float *f_translate, float *f_scale, bool setBlock)
{
    ...
    {
        glm::fvec3 p = voxelModel.midVoxel[i].locate;
        p.x = p.x - f_translate[0];
        p.y = p.y - f_translate[1];
        p.z = p.z - f_translate[2];
        // cout <<"\nnew " << p[0]<<", "<<p[1]<<", "<<p[2]<<endl;
        glm::fvec3 newCoordp = m_inverse * p; // 000000000000
        for(int i = 0; i < 3; i++){
            float tmp = 0;
            for(int j= 0; j < 3; j++){
                tmp += m_inverse[i][j]*p[j];
            }
            // cout << tmp<<" ";
            newCoordp[i] = tmp;
        }
        // cout <<"\nold "<< newCoordp[0]<<", "<<newCoordp[1]<<", "<<newCoordp[2]<<endl;
        ...
        for (int layern = 0; layern < layerNum; layern++)
        {
            if (voxelModel.midVoxel[i].layer == voxelModel.somInitLayer + layern)
            {
                ...
                if (!c)
                {
                    for (int layer = 0; layer < layerNum; layer++)
                    {
                        for (int block = 0; block < blockNum; block++)
                        {
                            ...
                            voxelModel.outvoxelnum[layer][block] = 0;
                ...
                if (newCoordp.x < 1.0 && newCoordp.x > 0.0 && newCoordp.y < 1.0 && newCoordp.y > 0.0 && newCoordp.z < 1.0 && newCoordp.z > 0.0)
                {
                    ...
                    voxelModel.outsomVoxel[layern][block][voxelModel.outvoxelnum[layern][block]].locate = {t_p.x, t_p.y, t_p.z};
                    findSurfaceVoxel(t_p.y, t_p.x, t_p.z, voxelModel.outvoxelnum[layern][block], layern, block, OUTSOM);
                    voxelModel.outvoxelnum[layern][block]++;
                }
            }
        }
    }
    // cout <<voxelModel.midVoxel.size()<<" : "<<voxelModel.voxelnum[0][0]<<", "<< voxelModel.outvoxelnum[0][0]<<endl;
    return c;
}
...
int RAWmodel_cls::SetBlockNum(const char *orient, int x, int y, int z)
{
    ...
    for (int blockn = 1; blockn < voxelModel.blockNum; blockn++)
        if (axis >= voxelModel.blockLocate[blockn])
            block++;
...
void RAWmodel_cls::GiveSpaceLocate()
{
    ...
    voxelModel.outsomVoxel = (USVoxData_t ***)malloc(layerNum * sizeof(USVoxData_t **));
    voxelModel.voxelnum = (int **)malloc(layerNum * sizeof(int *));
    voxelModel.outvoxelnum = (int **)malloc(layerNum * sizeof(int *));

    for (int layer = 0; layer < layerNum; layer++)
    {
        voxelModel.somVoxel[layer] = (USVoxData_t **)malloc(blockNum * sizeof(USVoxData_t *));
        voxelModel.outsomVoxel[layer] = (USVoxData_t **)malloc(blockNum * sizeof(USVoxData_t *));
        voxelModel.voxelnum[layer] = (int *)malloc(blockNum * sizeof(int));
        voxelModel.outvoxelnum[layer] = (int *)malloc(blockNum * sizeof(int));
        ...
        for (int block = 0; block < blockNum; block++)
        {
            ...
            voxelModel.outsomVoxel[layer][block] = (USVoxData_t *)malloc(layervoxelnum[layer] * sizeof(USVoxData_t));
            voxelModel.voxelnum[layer][block] = 0;
            voxelModel.outvoxelnum[layer][block] = 0;
            voxelModel.maxsize[layer][block] = {0, 0, 0};
            voxelModel.minsize[layer][block] = {10000, 10000, 10000};
            for (int j = 0; j < layervoxelnum[layer]; j++)
            {
                ...
                voxelModel.outsomVoxel[layer][block][j].color = {1.0, 0.7, 0.9};
                voxelModel.somVoxel[layer][block][j].locate = {0.0, 0.0, 0.0};
                voxelModel.outsomVoxel[layer][block][j].locate = {0.0, 0.0, 0.0};
                voxelModel.somVoxel[layer][block][j].texcoord = {0.0, 0.0, 0.0};
                voxelModel.outsomVoxel[layer][block][j].texcoord = {0.0, 0.0, 0.0};
                voxelModel.somVoxel[layer][block][j].textype = 0;
                voxelModel.outsomVoxel[layer][block][j].textype = 0;
            }
        }
    }
    pvoxelModel.maxsize = {0, 0, 0};
...
void RAWmodel_cls::setMaxbounder(int i, int j, int k, int layer, int block, int type)
{
    if (type == SHOWSOM)
    {
        ...
        if (voxelModel.maxsize[layer][block][0] < i)
            voxelModel.maxsize[layer][block][0] = i;
        if (voxelModel.minsize[layer][block][0] > i)
            voxelModel.minsize[layer][block][0] = i;
        if (voxelModel.maxsize[layer][block][1] < j)
            voxelModel.maxsize[layer][block][1] = j;
        if (voxelModel.minsize[layer][block][1] > j)
            voxelModel.minsize[layer][block][1] = j;
        if (voxelModel.maxsize[layer][block][2] < k)
            voxelModel.maxsize[layer][block][2] = k;
        if (voxelModel.minsize[layer][block][2] > k)
            voxelModel.minsize[layer][block][2] = k;
    }
    if (type == SHOWPSOM)
    {
        if (pvoxelModel.maxsize[0] < i)
            pvoxelModel.maxsize[0] = i;
        if (pvoxelModel.minsize[0] > i)
            pvoxelModel.minsize[0] = i;
        if (pvoxelModel.maxsize[1] < j)
            pvoxelModel.maxsize[1] = j;
        if (pvoxelModel.minsize[1] > j)
            pvoxelModel.minsize[1] = j;
        if (pvoxelModel.maxsize[2] < k)
            pvoxelModel.maxsize[2] = k;
        if (pvoxelModel.minsize[2] > k)
            pvoxelModel.minsize[2] = k;
    }
}
void RAWmodel_cls::findSurfaceVoxel(int y, int x, int z, int num, int layer, int block, int voxelType)
{
    ...
    if (z + 1 < infdata.resolution[0])
    {
        if (rawData[y][x][z + 1].layer != layer - 1)
        {
            ...
            if(voxelType == OUTSOM)
                voxelModel.outsomVoxel[layer][block][num].faceAir[0] = true;
        }
    }
    if (z - 1 >= 0)
    {
        if (rawData[y][x][z - 1].layer != layer - 1)
        {
            ...
            if(voxelType == OUTSOM)
                voxelModel.outsomVoxel[layer][block][num].faceAir[1] = true;
        }
    }
    if (x + 1 < infdata.resolution[1])
    {
        if (rawData[y][x + 1][z].layer != layer - 1)
        {
            ...
            if(voxelType == OUTSOM)
                voxelModel.outsomVoxel[layer][block][num].faceAir[2] = true;
        }
    }
    if (x - 1 >= 0)
    {
        if (rawData[y][x - 1][z].layer != layer - 1)
        {
            ...
            if(voxelType == OUTSOM)
                voxelModel.outsomVoxel[layer][block][num].faceAir[3] = true;
        }
    }
    if (y + 1 < infdata.resolution[2])
    {
        if (rawData[y + 1][x][z].layer != layer - 1)
        {
            ...
            if(voxelType == OUTSOM)
                voxelModel.outsomVoxel[layer][block][num].faceAir[4] = true;
        }
    }
    if (y - 1 >= 0)
    {
        if (rawData[y - 1][x][z].layer != layer - 1)
        {
            ...
            if(voxelType == OUTSOM)
                voxelModel.outsomVoxel[layer][block][num].faceAir[5] = true;
        }
    }
}
